...
reusable script: `scripts/computed-style-diff.mjs` (config 駆動)。
```bash
cd frontend && set -a && . ./.env && set +a   # auth (DOGFOOD_TEST_*) を環境へ
node ../.claude/skills/mockup-fidelity/scripts/computed-style-diff.mjs <config.json>
```
...
3. `computed-style-diff.mjs` 再実行 → 修正対象 pair が mockup と許容誤差内
