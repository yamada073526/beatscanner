...
- [ ] **Phase 0 — Scope & pre-flight**: mockup を full-read。実装 render path を特定。embedded↔standalone の構造差を明示 (mockup=フルページ / 実装=workspace ペイン)。**demo-only スキャン** (grep) で移植禁止要素を抽出。危険箇所列挙。
  - exit: Inputs 1-5 が user 確定済 + demo-only リスト確定 + element-map に未対応 mockup 要素を `excluded` 記録
...
- triaged AUDIT (`docs/specs/AUDIT_<screen>_<date>.md`、行 prefix `F`=fixed / `I`=intentional保全 / `D`=deferred-judgment / `X`=excluded-demo)
