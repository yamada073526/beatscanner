...
#### 3.3 content-audit 回帰ゲート (本番 content の known-pitfall 機械検査)

分析出力 (ガイダンス / 要約 / 市場の声 / AI図解 等) の **既知の content バグが回帰していないか**を本番に対して機械検査する。 runtime guard (例 `_guard_revenue_basis_mismatch` / sanitize layer) が壊れた / data source が drift した瞬間に検出する自動ネット。

```bash
bash scripts/content-audit-check.sh
# 重い visualize 検査も含める場合: CONTENT_AUDIT_FULL=1 bash scripts/content-audit-check.sh
```

**期待**: `✓ content-audit PASS` (exit 0)。 `✗ FAIL` (exit 1) なら **デプロイ非推奨 = ブロック扱い** → runtime guard を確認・修正。 pitfall を新規発見したら `content-audit` skill のレジストリ + 本 script に assert を 1 件追加して同期する。
