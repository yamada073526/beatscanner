...
`.github/workflows/vision_eval.yml` が main push 後に自動起動する。

**trigger**: `push: branches: [main]` (frontend/** 変更時) + `workflow_dispatch` (手動起動、任意 branch で実行可)

**secrets 設定方法**: GitHub repo → Settings → Secrets and variables → Actions → New repository secret
→ Name: `ANTHROPIC_API_KEY` / Value: `sk-ant-xxx` を追加。
不在時は capture のみ実行し、CI は落ちない (workflow は緑のまま)。

**artifact ダウンロード**: Actions タブ → 該当 run → Artifacts セクション → `vision-regression-<run_number>` を zip でダウンロード。
PNG 10 枚 + `vision-result.json` が含まれる。保管期間 30 日。

**rate limit 注意**: 本番 URL に対して demoAnalyze を 1 req 消費する (3 req/IP/day 制限)。
main push の多発時 (1 日 3 push 超) は workflow_dispatch でも消費されるので注意。
