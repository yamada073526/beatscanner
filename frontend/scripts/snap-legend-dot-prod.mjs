...
try {
  ...
  const disc = await p.locator('[data-testid="screener-grid-legend"] .disc').first().textContent().catch(()=>null);
  const hasDot = (disc||'').includes('ドットが付けば');
  console.log('legend contains ドットが付けば: ' + hasDot);
  console.log('disc: ' + (disc||'').replace(/\s+/g,' ').slice(0,140));
  await p.screenshot({ path: `${OUT}/legend-dot-prod.png` });
