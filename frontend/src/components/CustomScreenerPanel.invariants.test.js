...
describe('quiet-quality screener Sprint 3: quiet_quality preset (5軸・thesis型マッピング)', () => {
  ...
  it('PRESET_DISPLAY_CONDS.quiet_quality = 述語適用5軸 + uptrend opt-in override (隠れフィルタなし)', () => {
    // 述語適用5軸は上の hidden-filter invariant が自動走査。uptrend (上昇トレンドフィルタ A軸・SPEC_2026-07-02) は
    ...
    expect(PRESET_DISPLAY_CONDS.quiet_quality.sort()).toEqual(
      ['inst_qoq_calm', 'ocf_margin_pct', 'roe', 'rs_percentile', 'uptrend', 'volume_quiet'].sort(),
    );
  });
...
describe('uptrend filter (A軸): pv50/sl50 compound + quiet_quality opt-in override', () => {
  ...
  it('uptrend が CROW_LAYOUT に登録済 (DISPLAY_CONDS.quiet_quality が描画可能)', () => {
    const renderable = new Set(CROW_LAYOUT.flatMap((g) => g.keys));
...
describe('market-leading screener S4: market_leading preset (4段精度・範囲帯/null許容)', () => {
  ...
  it('PRESET_DISPLAY_CONDS.market_leading = 述語適用6条件と 1:1 (隠れフィルタなし)', () => {
    expect(PRESET_DISPLAY_CONDS.market_leading.sort()).toEqual(
      ['rs_mid_band', 'vs_spy', 'ocf_margin_pct', 'roe_lenient', 'eps_yoy_mid', 'latest_beat'].sort(),
    );
  });
