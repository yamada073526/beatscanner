...
describe('market-leading screener S4: market_leading preset (4段精度・範囲帯/null許容)', () => {
  ...
  it('PRESET_DISPLAY_CONDS.market_leading = 述語適用6条件 + uptrend opt-in override (隠れフィルタなし)', () => {
    // uptrend (上昇トレンドフィルタ A軸・2026-07-02 追記): quiet_quality と同じ opt-in override
    //   (default OFF・PRESET_PREDICATES 非登録) のため applied ⊆ display を壊さない。
    expect(PRESET_DISPLAY_CONDS.market_leading.sort()).toEqual(
      ['rs_mid_band', 'vs_spy', 'ocf_margin_pct', 'roe_lenient', 'eps_yoy_mid', 'latest_beat', 'uptrend'].sort(),
    );
  });
