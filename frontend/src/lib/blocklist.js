...
const BLOCKLIST_PATTERNS = [
  ...
  // (v124「追い風」 単独 match 過剰削除の教訓)。 backend prompt_negatives.py:BLOCKLIST_REGEX と 1:1 mirror。
  ...
  /(?:原動力|起爆剤|牽引役|推進力|けん引役)と(?:なる|なります|なろう|なるだろう|なるでしょう)(?!(?:中|ため|可能性|こと|べく|とされ|との|よう|懸念|ほど))/g,
  /(?:業績|株価|収益|売上|利益|成長)を(?:(?:押し上げ|支え)る|(?:牽引|後押し)する)(?:だろう|でしょう|見込み|はず|とみられ|と期待)/g,
  /(?:今後|将来|これから|来期|中期的に|長期的に)[^。]{0,15}(?:(?:牽引|けん引)(?:する|します|していく|し)|押し上げ(?:る|ます|ていく))/g,
  // ─── v219 §38 (resistance_retest 機能): リテスト/サポートの将来条件付き買い断定 ───
  // 「サポートまで戻れば反発/買い」「割り込まなければ買い」 型の将来断定 (金商法 §38)。
  // 静的辞書 narration とは別に DiagramCard LLM が price level を語る際の防御層。backend prompt_negatives と 1:1 mirror。
  // 条件形(戻れば/押せば/機能すれば)は動詞で変わるため汎用「ば」、加えて「なら/たら」 条件 + 肯定的帰結
  // (反発/上昇/買い)で捕捉 (Phase-gate §38 verdict: 「水準なら反発」「来たら買い」 型の漏れを塞ぐ)。
  // ※ 過剰削除回避: 「割り込めば/下抜けたら pattern failure・不成立」 (§38-safe な打消し文) は帰結が否定のため残す。
  /(?:リテスト|サポート|支持線|旧抵抗)[^。]{0,15}(?:ば|なら|たら)[^。]{0,10}(?:反発|上昇|買い)/g,
  /割り込ま(?:なけれ|ず)[^。]{0,10}(?:買い|上昇|反発)/g,
  // ─── FE=BAD-10 (breakout 機能 SPEC §2.5): ブレイク後 将来条件付き断定 (金商法 §38 第2号) ───
  // 既存 BAD-7 (上抜け/ブレイク.{0,10}買い)・v148 (青天井/まだ上がる) の未カバー領域だけを tight に塞ぐ。
  // backend prompt_negatives.py:BLOCKLIST_REGEX (BE=BAD-11) と 1:1 mirror (ID は装飾、mirror は regex で担保)。
  // ① pivot/抵抗線等 + 「抜けたら/超えたら/突破なら」+ 肯定帰結。打消し帰結 (不成立/pattern failure/下落) は
  //    肯定 alternation に含めず自動的に残す (§38-safe 両面記述を保護、Python 実測 hit=False)。
  /(?:pivot|抵抗線?|レジスタンス|高値|ネックライン)[^。]{0,12}(?:抜け(?:たら|れば|た[らな]ら?)|超え(?:たら|れば)|突破[^。]{0,4}(?:たら|れば|なら))[^。]{0,12}(?:買い|上昇|反発|一段高|上値)/g,
  // ② 「ブレイク(した) + 一段高/上値余地/買い場」型。BAD-7 未カバーの帰結語を補完。ブレイク単独では match させない。
