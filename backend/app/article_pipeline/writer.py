...
# 文体 anchor: Luxury Financial Editorial
- FT Weekend / Bloomberg Markets の「冷静な興奮」
- 熱量は **構造で表現** (反コンセンサス冒頭 → 数字 timeline → 業界対立 3 幕)
- 感嘆符・口語禁止、 体言止めは控えめ
- Aman 級「驚き・豪華・興奮・洗練」 のうち **洗練 + 豪華 + 興奮** target
  (静的記事なので「驚き」 は弱め)
...
# Markdown 構成 (deep_dive)
1. リード段落 (反コンセンサス angle 1 文 + 主張 1 文、 100-150 字)
2. ## 第 1 幕: 数字 timeline (source_facts の number カテゴリを 3-5 件、 時系列で)
3. ## 第 2 幕: 業界対立 / 競合 (proper_noun / causal を使い、 3-4 文)
4. ## 第 3 幕: 投資家への含意 (両論併記、 確率表現で 100-150 字)
...
<output>
{
  ...
  "body_md": "市場は NVDA の Q4 を「またビート」 と片付けたが、 数字の裏には供給制約の構造変化が潜む。\\n\\n## 第 1 幕: 数字\\nQ4 売上は $45.1B (+22% YoY) を達成 [1]。 注目すべきは Data Center 売上比率が 87.3% に達した点で [2]、 構造的な集中が進んでいる。\\n\\n## 第 2 幕: TSMC の動き\\nTSMC は CoWoS-S 生産能力を 2026 末までに 1.5 倍に拡張すると表明した [3]。 これは AI ASIC 各社の需要を捕捉する動きだが、 結果として NVDA の供給制約が緩和される可能性がある。\\n\\n## 第 3 幕: 投資家への含意\\n強気シナリオでは供給増による販売数量上振れ、 弱気シナリオでは Data Center 集中度の集中リスクが顕在化する。 1 社依存度を確認しながらポジションを判断したい。",
  "citation_indexes": [1, 2, 3]
}
</output>
</example>
...
async def write(
    *,
    ...
    body = await cli.complete(
        ...
        temperature=0.3,  # narration なので低 temperature だが完全 0 は単調になる
