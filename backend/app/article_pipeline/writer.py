...
- GOOD: 「本日は健全性フィルタ (時価総額・上場区分) を通過した銘柄のうち、 主要な開示イベント (8-K) ・ 大型増資 ・ M&A 等のニュースがあった銘柄を優先して N 件を選別しました。」
- GOOD: 「FMP gainers から時価総額・上場区分の健全性を確認し、 重要な企業イベント (増資 / M&A / 主要 8-K 開示) の有無と値動きを加味して N 件を選別しました。」
- 注: 選定基準は「どんな客観基準で絞ったか」 の事実説明に留める。 内部スコア値や「期待度」「上昇余地」 等の将来予測・順位付けは書かない (§38)。
