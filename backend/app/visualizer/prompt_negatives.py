...
NEGATIVE_EXAMPLES: list[dict] = [
    ...
    {
        # breakout 機能 SPEC §2.5。FE=BAD-10 / BE=BAD-11 (ファイル間の最終明示 ID が異なるため連番が割れる。
        # 1:1 mirror は BLOCKLIST_REGEX の pattern で維持)。
        "id": "BAD-11",
        "category": "ブレイク後 将来条件付き断定",
        "bad_output": (
            '"bullCase": ["pivot を抜けたら一段高", '
            '"レジスタンスを突破すれば買い", '
            '"ブレイクアウトしたので上値余地が大きい"]'
        ),
        ...
        # ⚠️ good_alternative は「上抜け/ブレイクアウト」+「上値余地/一段高/買い場」の複合を避ける (P2 regex に自己 hit するため)。
        # 「実績の事実記述」+「シナリオ帰結を『判断指標とされる』で受ける」+「否定帰結の両面記述」の3文構成 (Python 実測 hit=False)。
        "good_alternative": (
            '"bullCase": ["pivot を出来高 +52% を伴い終値で上抜け (直近確定事実)", '
...
BLOCKLIST_REGEX: list[re.Pattern] = [
    ...
    re.compile(r"(?:リテスト|サポート|支持線|旧抵抗)[^。]{0,15}(?:ば|なら|たら)[^。]{0,10}(?:反発|上昇|買い)"),
    re.compile(r"割り込ま(?:なけれ|ず)[^。]{0,10}(?:買い|上昇|反発)"),
    # ─── BE=BAD-11 (breakout 機能 SPEC §2.5): ブレイク後 将来条件付き断定 (金商法 §38 第2号) ───
    # frontend lib/blocklist.js:FE=BAD-10 と 1:1 mirror (ID は装飾、mirror は regex pattern で担保)。
    # ① pivot/抵抗線等 + 「抜けたら/超えたら/突破なら」+ 肯定帰結。打消し帰結 (不成立/pattern failure) は残す。
    re.compile(r"(?:pivot|抵抗線?|レジスタンス|高値|ネックライン)[^。]{0,12}(?:抜け(?:たら|れば|た[らな]ら?)|超え(?:たら|れば)|突破[^。]{0,4}(?:たら|れば|なら))[^。]{0,12}(?:買い|上昇|反発|一段高|上値)"),
    # ② 「ブレイク(した) + 一段高/上値余地/買い場」型。ブレイク単独では match させない (帰結語と複合のみ)。
    re.compile(r"(?:ブレイクアウト|上抜け)(?:した|済み)?[^。]{0,12}(?:一段高|さらなる上昇|上値余地|買い場|買い時)"),
]
