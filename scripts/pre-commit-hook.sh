...
for f in $STAGED_FLASH; do
    ...
    # §38 限定緩和 (ガイダンス履歴基盤 6体合議 §10 条件1、 案A 行注釈ホワイトリスト):
    #   「上方修正/下方修正」 は **前回会社ガイダンス比 (会社が自ら数値を変更した事実) に限り OK**。
    #   該当行に `@company-guidance-revision` 行末注釈がある場合のみ許可 (1 行 1 エントリの dict value 限定)。
    #   現コンセンサス比の文脈 (GUIDANCE_STATE_JP 等) への注釈付与は禁止 — 全面解除はしない。
    if echo "$ADDED" \
        | grep -vE '(禁止|含めない|出さない|使わない|BAN|NO-GO|@company-guidance-revision)' \
