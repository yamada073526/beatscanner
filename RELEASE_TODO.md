...
| ✅ 11-B-4 | **完了 (2026-05-07)** マクロニュース 話題順トグル + アテンションバッジ強化 — 6 体エージェントレビュー一致採用。`AttentionDots` を Lucide Flame + 数字 + 媒体 amber pill に進化 (cluster≥3 のみ表示)、ヘッダ右上に「話題順 / 新着順」segment toggle、localStorage `bs_news_sort_pref`、a11y 完全対応 (aria-pressed/focus-visible)。default 話題順 (HIGH ∩ cluster≥3 最上位 → cluster_size 降順 → published 降順)。24h 賞味期限は既存 fresh/stale 分割で吸収。残課題: PR Newswire bias (backend 改修)、初回トースト (UX 緩和)、相対閾値 (動的閾値) は §11-B-4-A に分離 | 完了 | 金融アナリスト (高 ROI 低コスト) |
| 🟠 11-B-4-A | **§11-B-4 残: PR Newswire bias 補正 + 初回トースト + 動的閾値** — 金融アナリスト指摘の PR ワイヤー (Business Wire / GlobeNewswire) は backend で source dedup 必要、CTR を歪める。マーケ指摘の初回トースト「並び順を 話題順 に変更しました」7 日表示。動的閾値 (上位 30%) は閑散日対応 | 1.5-2h | 金融 + マーケ |
| 🟠 11-B-20 | **マクロニュース タグ分類 Phase 2 マルチタグ化** — 6 体エージェントレビュー全員一致採用。backend `_classify_macro_news` の排他バグ (Fed+S&P 同時言及で「市場全体」タグを失う) 解消。`category: str` → `tags: list[str]` 化、frontend は OR フィルタ (`tags.some()`)。約 30 行差分 | 半日 | Web 開発指摘 (構造バグ) |
| 🟠 11-B-21 | **マクロニュース タグ分類 Phase 3 AI 自動分類 + センチメント** — Claude Haiku で `[マクロ, 金利, タカ派]` 等を自動付与。月コスト $5-15。ハト派/タカ派バッジ追加で「読まずに判別」体験。CTR +15% 期待。rule-based pre-filter (Fed/CPI 等) + LLM のハイブリッドで API コスト 60% 削減 | 2-3 日 | 6 体レビュー |
| ⭐ 11-B-22 | **【差別化最強】 マクロニュース タグ分類 Phase 4 影響セクター ETF 推定 + watchlist 連動** — マクロニュース → 影響を受ける ETF (XLF/XLE/SOXX/TLT 等) を自動表示、ユーザー保有銘柄セクターと一致するニュースを優先表示。**Bloomberg/Reuters にも無い差別化最強機能** (6 体レビュー全員一致)。じっちゃまプロトコル「マクロ→セクター→個別」思考順序と完全一致 | 3-5 日 | 6 体レビュー |
| 📣 11-B-22-AP | **【アピール戦略】 §11-B-22 Phase 4 差別化機能のユーザー訴求方法検討** — 競合他社にマネできない機能 (影響セクター ETF 推定 × watchlist 連動) を実装したら、LP/リリースノート/SNS でどう訴求するか戦略を立てる。同パターン他機能: (1) ポートフォリオのウェイト×決算日によるヒートマップ生成 (2) 5 条件クリアなどの買付条件による売買の採点機能。実装と同時にマーケ + UI/UX 両エージェントでアピール文面・スクショ設計・Product Hunt 訴求等を検討 | 各 0.5-1 日 | マーケ + UI/UX |
...
| 🟡 11-B-7 | **9-B-1 残: DnD 並び替え** (§9-B-1 既登録、`@dnd-kit/core+sortable+modifiers`、4-6h)。Phase 1A (UI 刷新、commit `6ab96d2`) 完了済、Phase 2 として DnD 実装予定 | 4-6h | 既存 |
| 🟡 11-B-7-B | **§11-B-7 連動: ウォッチリスト (リスト) + ポートフォリオダッシュボード のデザイン刷新** — ユーザー指摘 (2026-05-08)。Phase A 完了 (commit 97c83df + 59c7e52): bs-panel design token 統合 / ヒーロー大評価額 / SPY overlay + alpha バッジ / WatchlistSection filter bar 刷新 / Portfolio 常時表示でレイアウトシフト解消。残 Phase B/C/D は別エントリ (§11-B-7-B-Phase-B 等) で永続化 | 残 Phase | UI/UX + Web 設計 |
| 🟠 11-B-7-B-Phase-B | **§11-B-7-B 残: 買付クイック登録 (7→3 ステップ)** — 現状の手順「分析→ウォッチ→ホーム→観察→買付登録→保有→ポートフォリオ」を 3 ステップに短縮。`QuickAddHoldingModal.jsx` 新規 (株数+価格+日付の構造化 chip 入力) + DetailReport / ChartTab TickerRow hover / PortfolioDashboard 空状態の 3 箇所に「+保有」ボタン。watchlist 自動追加 + lots 統合。マーケ算定: 保有登録 CV +35-45%、NSM 直撃で D7 retention 改善の根本施策 | 1 日 | マーケ + Web 開発 |
| ⭐ 11-B-7-B-Phase-C | **§11-B-7-B 残: 売却損益累計 + 「自分の腕前」可視化** (差別化最強候補) — マーケ算定: **「実現損益累計 + 勝率 + ベスト/ワースト銘柄」を可視化する日本人向けアプリは皆無**。年末「2026 年成績」スクショ需要 (確定申告期 + 年初投稿) で **年 1 回必ずバズる季節性資産**になる。実装: `realizedPnL.js` (lots から FIFO 計算、月次 group by) + KPI に「実現損益 YTD: +$X」追加 + 月次バーチャート (緑/赤、折りたたみ) + 「勝率」「ベスト銘柄」「ワースト銘柄」ミニカード | 1-2 日 | 🔴 最強差別化 | マーケ + 金融 |
| ⭐ 11-B-7-B-Phase-D | **§11-B-7-B 残: 4 タスク統合 Killer Feature 2 本柱** (機会発見 Card + 課金キラー) — マーケ最重要分析: **(A) 注目銘柄スクリーナー = 課金キラー** (CV 直撃) + **(B) Beat 連勝銘柄ランキング = SNS バズ** (集客直撃) の 2 本柱で役割分担。残 2 タスク (会社紹介長文 strainer 風 / アナリストの視点) は補助情報として配置。「機会発見 Card」(Stripe Insights 流) を Portfolio 横に常時表示、煽らないトーンで「Beat 連勝の事実提示」のみ (じっちゃま長期 buy-and-hold 思想衝突回避、金融指摘) | 3-5 日 | 🔴 最強差別化 | マーケ + 金融 |
| ✅ 11-B-8 | **完了 (2026-05-07)** セクション順序最適化「Your portfolio first」 — `useNewOrder = !!user && watchlist.length > 0` で条件分岐: ログイン済+watchlist≥1 は ウォッチリスト→ChartTab→Movers→マクロ→経済指標、それ以外は旧順序。ChartTab Suspense fallback に `min-height: 480px` (CLS 0)、上位 3 銘柄に `prefetchAll` fire-and-forget。5 体エージェントレビュー全員一致懸念 (空 watchlist DAU 低下 / CLS) を解消 | 完了 | Web 設計 Q5-2 |
| ✅ 11-B-15 | **完了 (2026-05-07)** 2 グループ化 (Apple Stocks / Linear 流) — `あなたの銘柄 (ウォッチリスト + ChartTab + Movers)` / `マーケット (マクロ + 経済指標)` の 2 群に分離。境界は 1px hairline + 11px ラベル (letter-spacing 0.08em)。useNewOrder=true 時のみ表示、旧順序ではフラット。4 体エージェントレビュー全員一致採用 (3/4 が Movers を自分ごと側帰属、CV CTR +8〜12% 期待)。実装は `.home-group-divider` + `.home-group-label` クラス (index.css) | 完了 | ユーザー直感 + 4 体レビュー |
| 🟠 11-B-16 | **§11-B-8 残: High Impact 日のマクロ自動昇格** — 金融アナリストエージェント提案。EconomicCalendar に当日 high-impact (CPI/FOMC/NFP/PPI) を含む場合のみマクロを最上部に動的昇格。寄り付き前ユーザーの最重要情報見落としリスクをゼロに。impact フラグを backend `_EVENT_NAME_JP_MAP` に追加要 | 1.5h | 金融アナリスト |
| 🟠 11-B-17 | **§11-B-8 残: MoversCard を ChartTab の上に再配置** — UI/UX エージェント提案。MoversCard は軽量カードで「2 秒で今日何が動いたか把握」できる。ChartTab より上に置くと first viewport の情報密度が上がる。要 A/B テスト | 30 分 | UI/UX |
| 🟠 11-B-18 | **§11-B-8 残: A/B テスト計装** — マーケター + Web 設計合同提案。`section_view` (50% 可視 IntersectionObserver) / `section_cta_click` / `first_meaningful_action_time` / `watchlist_size_at_session_start` を Supabase に記録。順序を Supabase feature flag 化して 50/50 で 2 週間。主要 KPI = D7 retention / 銘柄クリック CTR / session length | 3-5h | マーケター + Web 設計 |
| 🟠 11-B-19 | **§11-B-8 残: マクロ横スクロールカルーセル化** — UI/UX エージェント提案。TodaysBriefSection を横スクロール 1 行 (高さ 1/3 圧縮) にしてスクロール疲れと「毎日開きたくなる」原則を両立。モバイルで効果大 | 1h | UI/UX |
...
| 11-B-14 | **NewsRow の「HIGH · マクロ」表記の冗長性解消** — 現状「マクロ」「地政学」タブは全件 HIGH、「市場全体」タブは全件 MED で、pill 内の `HIGH ·` プレフィックスがほぼ冗長。FMP 有料化で件数増 → HIGH/MED 混在 が生まれる段階で再判断。推奨: `⚡`/`🔥` アイコン + 文字削除。代替: 完全削除 / 「重要」日本語化 (UI 内部リテラシー次第) | 30 分 | ユーザー指摘 (2026-05-07) + FMP 有料化後の再評価項目 |

### 11-C. P2 戦略 (FMP 有料化と同期、ROI 最高、合計 1-2 週)

| # | 内容 | コスト | ROI | 出典 |
|---|---|---|---|---|
| ✅ 11-C-1 | **完了 (2026-05-07)** Phase 1 構造化データ JSON-LD (半日実装) — 6 体エージェントレビュー + 2026 BP 統合採用。実装: `WebSite` + `Organization` + `Event ItemList` (JSON-LD) + 静的 og:image (SVG プレースホルダ) + Twitter Card + canonical + `llms.txt` + `sitemap.xml` + `robots.txt` (AI bot allow)。useEffect 注入 (StrictMode 二重実行対応の id 重複ガード)、純関数 `jsonLdBuilders.js` で生成。**NewsArticle は重複コンテンツ penalty リスクで意図的に除外** (金融/マーケ全員一致)。Phase 2 (Backend Jinja2 移行) のトリガー条件: Search Console 4 週で表示 < 10 / 月 / Bingbot/Perplexity 流入を狙う方針決定 | 完了 | 🔴 最高 | マーケター |
| 🟠 11-C-1-A | **§11-C-1 残: og-image.svg → PNG 化** — 現状は SVG プレースホルダ (`/og-image.svg`)。SVG は modern Google / Twitter 対応だが、Facebook / LinkedIn / Discord は PNG/JPEG 必須。Figma で 1200×630 PNG export → `/og-image.png` 配置 + index.html の og:image / twitter:image を `.png` に変更 | 5-15 分 | UI/UX |
| ✅ 11-C-2 | **完了 (2026-05-08) Phase 1A** OGP 動的生成 (Pillow + Railway cron) — 6 体エージェントレビュー全員一致採用。実装: backend `og_image_generator.py` (Pillow ベース、4x supersampling + LANCZOS、favicon と完全一致のロゴ詳細描画)、`/api/og-image-today.png` endpoint (24h memory cache)、Railway cron 毎日 ET 6:00 (= UTC 11:00) で再生成、3 段 fallback (動的→直近成功→1x1)。`_shorten_event_name()` で経済指標を CPI/FOMC/NFP 等の略称に自動変換 (regex 抽出 + 28 件の日本語短縮 dict)。ヘッダーリブランド同梱 (ロゴ 28px / Medium / BeatScanner キャメル統一) | 完了 | 🔴 最高 | マーケター |
| 🟠 11-C-2-B | **§11-C-2 残: OGP デザイン全体ブラッシュアップ** — ユーザー指摘「カード全体のデザインがあまりパッとしない (格好よくない)」。現状はミニマル過ぎる印象。改善候補: (1) Bloomberg/TradingView 流のチャート風背景装飾、(2) より大胆なタイポグラフィ階層 (主見出し 100pt+)、(3) ローソク足 / グリッドパターンの強化、(4) 「速報感」を伝える緊急性表現、(5) センチメント色 (緑/赤 で「市場 sentiment」表示)、(6) 銘柄サムネ画像との合成 (per-news OGP の前段階)。サブエージェントレビュー必須、Figma モック作成推奨 | 1-2 日 | UI/UX + マーケ |
| 🟠 11-C-2-C | **§11-C-2 残: 寄付き後 ET 11:00 2 回目 cron** — actual 値 (CPI/FOMC 発表後の数値) を反映するため。現状は朝 6:00 の予想値ベース。2 回目 cron で actual を入れると AI 検索引用機会↑ | 30 分 | 金融アナリスト |
