...
| 🟠 11-B-15 | **§11-B-8 残: 3 グループ化 (segment divider)** — Web 設計エージェント提案。`Your positions` (Portfolio + ヒント) / `Your watchlist` (ウォッチリスト + ChartTab) / `Market` (Movers + マクロ + 経済指標) の 3 群に segment divider + group label。Wall of cards 問題解消、Linear/Stripe 流の高品質感。順序を `const HOME_SECTIONS = [...]` 配列に外出し、`.map()` で render すると 1 行差分 revert 可能 | 2h | Web 設計 |
