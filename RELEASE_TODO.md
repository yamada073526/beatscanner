...
| 🟠 11-B-7-B | **§11-B-7 連動: ウォッチリスト (リスト) + ポートフォリオダッシュボード のデザイン刷新** — ユーザー指摘 (2026-05-08): Phase 1A で ChartTab がホテルロビー級洗練になった結果、連携している HomeTab.jsx の watchlistSection (タグフィルタ + 保有/観察 segment) と PortfolioDashboard.jsx が見劣りする。統一されたデザイン言語で刷新 + 機能連携の快適さ向上 (タグ操作 → ChartTab フィルタ即反映 / 保有モード切替 → ChartTab ハイライト連動 等)。Phase 1B / Phase 2 完了後に着手 | 1-2 日 | UI/UX + Web 設計 |
