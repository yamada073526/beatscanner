...
## 7.9 モックアップ v7（アイコン刷新 + プロ点検の残改善まとめ 2026-06-22）

### アイコン（模範解答 PDF と同水準）
- 4プリセットのアイコンを **Lucide 公式パス（MIT）に刷新**: 決算合格=`badge-check`（認証シール）/ 新高値ブレイク=`trending-up` / 旬のセクター=`layout-grid`（1セル強調）/ セクター別リーダー=`crown`。stroke 1.9。タイル=シアン下地・選択/hover でアイコンが gold 発光（v5 の方針維持）。

### 残改善（Sonnet UI/UX 点検で抽出→反映）
- **verdict 常時緑を撤廃**: detail に `vtype`、p1=pass(緑)/p2・p4=neutral(gold)。
- **必須ゲートはトグル不可**（lock アイコン表示）に。`calcCount` のゲートOFFで件数増の論理矛盾を解消。
- **focus-visible / aria**（role=radio/switch、aria-pressed/checked、gold outline）でキーボード操作対応。
- **レスポンシブ**（presets 4→2→1、md 1カラム、conds 1カラムの breakpoint）。
- **ブレイク chip のシアン → gold**（色ルール: シアンは方向性に使わない）。
- **sort を `<select>` ネイティブ→ appearance:none + カスタム矢印**でブランド統一。
- **旬のセクターの master を空にしない**: 左=セクター一覧（クリック）→ 右=そのセクターの Top3。
- **文言重複の解消**: fhint を Pro 訴求の反復から「データ出典 + 必須ゲート注記」に差替。
- **disclaimer を結果の下へ移動**、seasonchip は p4「常時」を neutral 配色に、prefers-reduced-motion に hover transform も含める、行クリックで詳細連動（モック）。

モックアップ: `docs/specs/mockups/screener-strategy-presets-v7.html`（最新）。
