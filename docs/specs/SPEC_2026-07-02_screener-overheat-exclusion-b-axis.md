...
| 閾値グリッド (緩/標/厳/最厳) | **未確定 (Sprint 1 待ち)** | 実データ較正が必須前提 |
...
Sprint 1 (実データ較正) は本セッションの network policy 制約 (本番 API 直接アクセス不可・FMP key なし) のため一時 GH Actions workflow (`_tmp_b_axis_calibration.yml`, PR #179 merge・実行後削除予定) を組んで実施した (下記 §12)。

---

## 12. Sprint 1 実データ較正結果 (2026-07-02 完了)

### 実測方法
一時 workflow (`workflow_dispatch`、run 28557212045、5分で完了) が本番 `/api/scanner/universe` から quiet_quality[標準]/market_leading[標準] 通過銘柄を抽出し、既知 B cohort (MU/WDC/STX/STRL) を強制的に対象へ加えた上で `/api/price-history/{ticker}?period=1y` から dd60 (直近60営業日高値からの下落率)・runup60 (その高値に至るまでの直近60営業日の上昇率) を実測 (対象118銘柄)。

### 母集団
- quiet_quality[標準] = **69件** (as_of 2026-07-01 universe cache)。既知 cohort 4銘柄全て在籍。
- market_leading[標準] = **61件**。既知 cohort は **0件** (RS 96〜98 が対象の中位帯45〜75外のため元々非対象)。
...
**dd60 単独 (S-1) では分離不可能**: quiet_quality healthy(非cohort)群の dd60 は min=−40.5〜max=0.0 (median −6.9) と cohort の値域 (−14.9〜−21.9) を完全に包含しており、dd60 単独の閾値では健全な深い調整 (APA −25.8% 等、runup 無し) を誤って巻き込むか、cohort を取りこぼすかの二択になる。→ **A軸 SPEC の懸念が実データで裏付けられ、S-2 compound の必要性が確定した**。

**runup が真の分離軸**: cohort の runup60 は +160〜+277% (中央値+199%) という極端な値。quiet_quality healthy 群の runup60 中央値は +46.3% (p75=+61.5%) で、cohort とは明確に水準が異なる。dd60 と runup60 を AND で組み合わせることで「大きく吹き上げてから大きく崩れた」パターンのみを精密に捕捉できる。

### 追加の発見: INOD・CRDO は「誤除外」でなく「未知の追加陽性」の可能性
compound 閾値で known cohort と一緒に捕捉される `INOD`(dd60=−40.6%, runup60=+252.7%) と `CRDO`(dd60=−14.4%, runup60=+244.5%) は、既知 cohort と同型の「巨大急騰→大幅反落」パターンを示しており、既知4銘柄に限定されない汎化性能の傍証と判断する (誤除外でなく追加の真陽性の可能性が高い)。一方 `ALAB`(runup60=+312.3% だが dd60=−10.8%=ほぼ高値圏) は「まだ崩れていない」健全な強い株として正しく温存される。`NXPI`/`APA`/`CF`/`LPG` 等 (深い dd60 だが runup60 が低水準) も健全な調整として正しく温存される。

### 確定閾値グリッド (4段階・実データ較正済み)

| 段 | 閾値 (除外条件: dd60 < X **かつ** runup60 ≥ Y) | cohort捕捉 | qq追加除外 | ml追加除外 | qq残 (69→) | ml残 (61→) |
|---|---|---|---|---|---|---|
| 緩 (loose) | dd60 < −20% かつ runup60 ≥ 140% | 1/4 (STRL) | 1件 (INOD) | 0件 | 67 | 61 |
| 標準 (standard) | dd60 < −16% かつ runup60 ≥ 140% | 3/4 (STRL/STX/WDC) | 1件 (INOD) | 0件 | 65 | 61 |
| 厳 (strict) | dd60 < −14% かつ runup60 ≥ 140% | **4/4 全捕捉** | 2件 (CRDO/INOD) | 0件 | 63 | 61 |
| 最厳 (severe) | dd60 < −12% かつ runup60 ≥ 80% | 4/4 全捕捉 | 4件 (+NTAP/POWL) | 2件 (COCO/NEXA) | 61 | 59 |

各段で単調逓減 (A軸と同じパターン)。**厳 (strict) が「既知の4銘柄全てを確実に捕捉する最小の閾値」**であり実用上の primary candidate。

### market_leading への影響について (誠実な報告)
現時点の market_leading[標準] 母集団には B cohort が0件 (RS帯の対象外)、かつ healthy 群の runup60 上限が+81.9%と低いため、緩〜厳の3段階では **market_leading の件数に一切影響しない** (61→61)。最厳段でのみ2件除外。これは「B軸が market_leading に効いていない」のではなく、「market_leading (RS中位帯の出遅れ回復株) は元々 B cohort のような極端モメンタム銘柄をほぼ含まない」という preset の性質上自然な結果であり、将来 market_leading にモメンタム銘柄が混入するケースへの安全網として機能する設計は妥当と判断する。

### gate1 再承認依頼
上記グリッドを Sprint 2 (backend実装) の確定閾値として採用してよいか、user 承認を仰ぐ。
