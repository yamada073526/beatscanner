...
#### (4) breakout transition の email dispatch 方針 — **初期 sprint は DEFER**

> **C1(Anthropic engineer 指摘)** — `_detect_signal_transitions` の唯一の caller は `cron_cup_notify`(main.py:L16652、引数なし = `pattern_type="cup_handle"` 固定)で、funda 5PASS filter + per-user dedup + Resend digest mail を駆動する。breakout transition の dispatch を誤配線すると funda filter/dedup を bypass した breakout メール or §38 非準拠ラベルがメール本文に出るため、以下の方針を明文化する。

**(a) dispatch 先 — 専用 `/api/cron/breakout-notify` を後続 Phase で新設する。**
`cron_cup_notify`(L16652)を直接拡張しない。理由: cup notify 内部は `_detect_signal_transitions` のデフォルト引数(`"cup_handle"`)で呼び出しており、拡張するには引数変更と dispatch ループの両方に手を入れる必要がある。breakout は `pattern_type="breakout"` / `_BO_TRANSITION_MAP` と cup とは別 namespace のため、**`_detect_signal_transitions("breakout")` を別途呼ぶ専用 cron を新設する**方が回帰ゼロ。cup digest mail のテンプレート(`send_cup_handle_digest`、`_TRANSITION_LABEL`、`SUBJECT_TEMPLATE`、`DISCLAIMER_HTML` / `DISCLAIMER_TEXT` いずれも `backend/app/mailer.py`)は cup_handle の文言で構成されており、breakout transition に流用すると §5 (景表法) 虚偽回避の面で不適切。
**retest(resistance_retest) の配信が B で DEFER(main.py L19053「配信は B で DEFER」)されている現状と順序整合を取り、breakout の email 配信も screener + viz 着地(Phase A)の後続 Phase とする。** 実装着手まで `_detect_signal_transitions("breakout")` を cron から呼ばない。

**(b) merge vs 別メール — 配信着手時は別メール(別 cron)で新設し、cup digest とは独立する。**
同一ユーザーに 1 通 digest に統合する設計は将来の拡張として検討できるが、初回実装は cup の `send_cup_handle_digest` を breakout に流用しない。専用の `send_breakout_digest`(仮称)を `backend/app/mailer.py` に新設し、`SUBJECT_TEMPLATE` / `DISCLAIMER_HTML` / `_TRANSITION_LABEL` に相当する **`BREAKOUT_SUBJECT_TEMPLATE` / `DISCLAIMER_HTML_BREAKOUT` / `BREAKOUT_STATE_LABEL_JP`** を独立定義する。`DISCLAIMER_HTML` は「Cup-with-Handle パターン」と明記しており、breakout メールに流用すると §5 の訴求文言と実装の乖離(Trust Cliff)になる。

**(c) §38/§5 + filter 遵守 — 配線時は funda 5PASS filter + per-user dedup を必ず通す。**
`_is_ticker_funda_pass`(main.py:L16521) および `_is_already_dispatched`(L16613、`pattern_type` を明示して呼ぶ)を cup と同様に通す。本文ラベルは `BREAKOUT_STATE_LABEL_JP`(専用静的辞書)のみ使用し、cup の `_TRANSITION_LABEL`(main.py:L16515 近傍、`formation_to_breakout_pending` / `breakout_pending_to_confirmed` の cup 語彙)を breakout メールに流用しない。**`bo_pending` は nightly 非保存(§4.1)のため transition が発生しない — screener 同様、email でも pending は配信されない**。`notification_dispatch_log` insert 時は `pattern_type="breakout"` を明示(`_is_already_dispatched` の `pattern_type="cup_handle"` ハードコードは cup 専用のため、breakout は別途 `pattern_type` を渡す形で呼ぶ)。

**触ってはいけない箇所**: `cron_cup_notify`(L16652)は変更しない。`send_cup_handle_digest`(mailer.py:L195) / `_build_digest_html`(L123) / `DISCLAIMER_HTML`(L83) / `_TRANSITION_LABEL`(L104)は cup_handle 専用で不変。
...
3. **freshness assert(⑩ 第一手 = silent fail 是正)— Sprint 0 で実装 + 検証で前提是正済(2026-06-16)**

   ⚠️ **Sprint 0 検証で判明した重要事実(SPEC 旧前提の訂正)**: Railway gateway は scan 応答を ~5min で打ち切り **HTTP 502 を返すが、backend の scan は完走し DB に persist する**(検証実測: cup-scan が 20:37 UTC に 502 応答 → `pattern_signals.scanned_at` は 20:40 UTC に更新)。つまり **502 ≠ データ stale**。帰結: (a) scan 応答 body は 502 エラーページで **JSON parse 不能** = 「response の `breakout_detected_count/upserted` を jq で読む」旧設計は機能しない(body 空で常に 0)。(b) HTTP status は健全性の信号にならない。`continue-on-error` silent fail の真の問題は「HTTP 緑に埋もれる」でなく「**DB が実際に古いか**」であり、判定は **read endpoint の DB date を真とする**。

   **採用した設計(`.github/workflows/nightly_scan.yml`、Sprint 0 実装済・commit f8fa840)**:
   - 各 scan step: HTTP 非200 は **`::warning::`**(gateway timeout の可視化に留め、job は止めない)。`http/produced/upserted` を `GITHUB_OUTPUT` に出力。
   - 末尾「**Freshness gate**」step(`set +e`): read endpoint の DB date を **`>= 前日`** で判定(scan-date/market-date の曖昧さを避け、2日以上古ければ真の stale = `exit 1`)。
     - `rs.calc_date`(`/api/scanner/rs`)/ `canslim.as_of`(`/api/scanner/canslim`)/ `retest.signal_as_of`(`/api/scanner/retest` = cup-scan の `_scan_one` が同時保存する **cup-scan freshness の proxy**)を `chk` 関数で判定。
     - cup-handle endpoint は日付を返さないため `total>0` の sanity warning のみ(retest proxy が cup-scan freshness を担保)。
     - HTTP 200 応答時のみ `produced>0 かつ upserted=0`(GRANT persist 失敗 = データ喪失)を `exit 1`(502 時は body 空で `produced=0` = 対象外)。
   - **検証**: 実データ dry-run + workflow_dispatch 2 回で healthy=緑 / 旧 HTTP-gate=赤 を確認(§4.5 検証ログ)。

   **breakout 配線時(Sprint 2)の追加**: `/api/scanner/breakout` に `as_of`(scan date)を返す設計にし、Freshness gate の `chk` に `chk "breakout as_of" "$BASE/api/scanner/breakout?limit=1" ".as_of"` を1行追加する。**cup-handle endpoint にも `as_of`/`scanned_at` を返す改修が望ましい**(現状 retest proxy 依存。小改修なので別 backlog)。

   **⑩ silent fail 是正の位置付け**: nightly 502 の根本対策(universe 分割 / scan 非同期化、§4.5-4)は breakout 実装とは**独立した workstream**。だが Sprint 0 検証で「502 でも persist する」と判明したため、502 の**緊急度は当初想定より低い**(毎晩 502 でもデータは大抵 fresh)。Freshness gate が真の stale を捕捉するので、universe 分割/非同期化は「502 が backend 完走前に落ちる頻度が上がったら」着手する後続課題に降格(breakout 配線の hard 前提ではなくなった)。
...
- [ ] **breakout email dispatch は後続 Phase(DEFER)** — 初期 sprint では `_detect_signal_transitions("breakout")` を cron から呼ばない。配線着手時は専用 cron `/api/cron/breakout-notify` を新設し、`cron_cup_notify`(main.py:L16652)を拡張しない。mailer は `BREAKOUT_STATE_LABEL_JP` / `BREAKOUT_SUBJECT_TEMPLATE` / `DISCLAIMER_HTML_BREAKOUT` を独立定義(cup の `send_cup_handle_digest` / `_TRANSITION_LABEL` / `DISCLAIMER_HTML` は cup 専用で流用禁止)。`_is_ticker_funda_pass`(L16521) + `_is_already_dispatched`(`pattern_type="breakout"` 明示)(L16613) を必ず通す。`bo_pending` は nightly 非保存のため dispatch 対象外。retest B(配信 DEFER)と同じ後続 Phase 扱いで順序整合(§4.2-A (4) 参照)。
