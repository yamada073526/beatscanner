...
def _classify(pivotH):
    ...
    return ("bo_low_vol", round(vol_ratio, 2))   # 終値抜けたが出来高 <1.3x
```
...
| `bo_low_vol` | close > pivotH | < 1.3x | 出来高伴わず = だまし上抜けリスク高。弱tier 表示(§38: 事実のみ) |
...
return {
    ...
    "tier": tier,                            # "confirmed" | "soft" | "low_vol" | "pending"
    "polarity": polarity,                    # "up"(confirmed のみ, vol≥1.5x) | "neutral"(soft/pending/low_vol/extended)
    ...
    "is_new_52w_high": is_52w,               # strong tier badge (§1.6)
    "is_extended": is_ext,                   # §1.4 過熱フラグ
    ...
    "levels": [                              # frontend PriceLadder 接続
        ...
        {"kind": "high_52w",   "price": prior_252w_high, "label": "52週高値"},  # is_52w 時のみ
    ],
}
```
...
緑(`--color-gain`)は **過去/現在の確定 polarity のみ**。**confirmed = 終値 + 出来高 ≥1.5x で実際に pivotH を抜けた確定事実 → `polarity="up"`(緑可)**。**soft(1.3–1.49x)/pending/low_vol/extended は `neutral`(緑禁止・§9 #9 是正で soft の緑可を撤回)**。hex 直書き禁止(`design_system.md` token)。
