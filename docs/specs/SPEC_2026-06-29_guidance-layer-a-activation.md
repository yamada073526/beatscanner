...
- [x] Phase 2: merge → deploy → nightly canslim で guidance_source='8k' > 0 + 本番 snap ●確認 + §38 per-source verify (2026-07-02・下記「§38 per-source verify 実測結果」参照)。
...
## §38 per-source verify 実測結果 (2026-07-02・DoD 最終項目)

> Phase 4 (2026-07-01・MU/JBL/SNX/BB/CNXC 全件 PASS) の自己申告を鵜呑みにせず、本番 DB
> (`guidance_snapshots` × `consensus_snapshots`) の生データから surprise% を独立に再計算し
> 突合した (SQL のみ・main.py 不触)。

### 検証対象の確定
本番 `screener_fundamentals.guidance_source='8k'` が実際に立っている銘柄は **5 銘柄** (BB / CNXC /
JBL / MU / SNX)。引き継ぎ指示にあった「14 銘柄」はコードベース・DB のどこにも根拠が見つからず誤り
(出所不明・おそらく前セッションの hallucination) と判断。参考: `guidance_snapshots` 内で quarter 型
ガイダンス行を持つ「Layer A 候補」銘柄は 16 (AMZN/ANDG/BB/CBRS/CCL/CNXC/JBL/KBH/KFY/LZB/MLKN/MU/
NVDA/PRGS/SNOW/SNX) だが、うち PIT consensus が実際に成立するのは上記 5 銘柄のみ (残りは構造的 PIT
限界 §「受容する限界」により不成立、想定通り)。

### 独立再計算 (`_compute_layer_a_surprise` のロジックを手計算で再現・全件一致)

| ticker | 指標 | guidance mid | PIT consensus | 再計算 | DB 格納値 | ガード |
|---|---|---|---|---|---|---|
| BB | EPS | 0.035 | 0.0392288 | 0.0% | 0.0% | range 幅ガード (consensus が [0.03,0.04] 内) |
| BB | Rev | 142.5M | 137.49M | 0.0% | 0.0% | range 幅ガード (consensus が [137M,148M] 内) |
| CNXC | EPS | 2.71 | 3.085 | -12.16% | -12.2% | 一致 (range 外) |
| CNXC | Rev | 2.4775B | 2.5325B | -2.17% | -2.2% | 一致 (range 外) |
| JBL | EPS | 4.0 | 3.71415 | +7.70% | +7.7% | 一致 (range 外) |
| JBL | Rev | 9.6B | 8.9733B | +6.98% | +7.0% | 一致 (range 外) |
| MU | Rev | 50B | 42.6875B | +17.13% | +17.1% | 一致 (EPS は eps_basis=gaap で正しく抑止=null) |
| SNX | Rev | 18.6B | 16.827B | +10.54% | +10.5% | 一致 (EPS は同様に抑止) |

PIT の look-ahead 禁止 (`snapshot_date < filed_at`) と staleness (全件 filed_at の 1 日前以内・10 日
stale guard 内) も実データで確認済み。**8 指標全件で DB 格納値と再計算値が一致、ガード (range 幅 /
GAAP basis 抑止) も設計通り発火**。

### 未検証スコープ (環境制約)
EDGAR 原典 (8-K 本文) はこのセッションの network policy から到達不可 (`sec.gov` も 403 でブロック)
のため、抽出数値と一次資料の逐語照合はこのセッションでは実施できていない。この部分は Phase 4
(2026-07-01・同一 5 銘柄) の verify に依拠する。原典照合が必要な場合はネットワーク制限のない環境
(ローカル PC 等) で `source_url` 列を直接 curl して再確認すること。
