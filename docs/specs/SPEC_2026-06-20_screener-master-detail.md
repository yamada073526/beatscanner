...
1. **funda_pass が疎** (data 実態、 bug でない): `earnings_evaluation` は `max_eval_date=2026-05-15`・全期間 `all_passed=true` 25 件・**95日窓 0 passers** (5 条件は pass 率 0.7%、決算シーズン谷間)。既存 `cup-handle` scanner も同依存で現在 0 件。→ funda_pass を tri-state 化済 (477c202)。**設計論点 (Sprint 3)**: universe-wide で常時新鮮な「ファンダ」次元は CAN-SLIM 数値 (screener_fundamentals, 06-19 新鮮) 側。じっちゃま 5 条件 binary は決算イベント依存の sparse facet として「最新決算で5条件達成」と明示するか、CAN-SLIM 数値 facet を主にするか要決定。
   - **→ 決定 (2026-06-21, user 明示)**: **ファンダ次元は専用セッションまで据え置き**。「じっちゃまのファンダ条件を BeatScanner にどう落とし込むか」は KB (`/Users/yamadadaiki/Projects/investment-knowledge-base`) を**正**として別途専用セッションで設計する難問のため、本セッションでは触れない。`funda_pass` を実装都合で grades (EPS成長/ROE) に**すり替えるのは禁止** (2026-06-21 に一度誤って実施→revert 済。決算サプライズ超過 ≠ 成長率/ROE で投資判断の意味が別物、かつ「決算3条件」表記と中身が乖離する Trust Cliff)。**それまでの暫定**: Pane3 idle hero「今日の筆頭」は `funda_pass` を交差に含めず **「RS × テクニカル (Cup/ブレイク)」のみ**で運用 (eyebrow/tooltip/説明からも「決算3条件」を除去)。ファンダ復活時はこの ADR と [[reference_jijima_investment_criteria]] を再読。
...
### 0-8. Sprint 4-6 UI/UX 改修 要件 (2026-06-20 user dogfood + grill-me)

**user 主訴 (Sprint 3 dogfood)**: 「パラメータはおおむね希望通り。だが UI/UX が使いづらい。**各要素の重みが分からず、どこを見たらいいのかパッと見で分からない**」= 視覚的ヒエラルキー/element weight 不在 (全要素が同格)。「デザインルールはアップグレード済 → 検索設定改修が済んだら UI/UX 改修を」。→ Sprint 4 (リスト密度・上位強調下位後退・固定列) + Sprint 6 (shadowゼロ視覚言語・weight3段・余白3段) の核心。

**grill-me で確定した視覚ヒエラルキー (4 決定、Sprint 4 実装の SSOT)**:
1. **主役 = 絞り込んだ銘柄リスト**。フィルタ群は上部の操作帯に格下げし、銘柄行を視覚の重心に (原則2「毎日開きたくなる」/4「人力代替」)。現状の「全要素同格」を解消する最上位の決定。
2. **既定の並び順 = 条件合致度順** (各 facet の超過幅の合計/正規化等、§38 中立な事実ソート)。上=最初に目が行く。「条件充足」は事実表示で色断定しない (緑禁止)。「で、どれ?」に即答。
3. **操作部 = 「厳しさ(緩/標/厳) + 適用中サマリ + 『詳細を開く』ボタン」の1行に集約**。sector/mcap/grade override (詳細展開 accordion)・locked 鍵は「詳細」を押すと展開。初期画面は銘柄リストが大半を占める (chip の壁が主役を埋める現状を解消)。
4. **銘柄行 = ロゴ+ティッカー(会社の顔)を最大要素に**。会社名は小、合致条件はミニバッジで副、数値(RS/EPS)は右端に控えめ。**上位数件は太字/やや大、下位は淡く小さく (上位強調・下位後退)** で起伏を付ける。

**追加バグ (quick-win、Sprint 4 で修正)**: ペイン2上部の市場局面バナー (`FtdRegimeBanner`) が途中で truncate されている (「市場局面 上昇トレンド入りのシグナル点灯 NASDAQ で Follow-Through Day …」で切れる) → 全文表示されるよう修正 (line-clamp/ellipsis 解除 or 折返し)。

**未決 (Sprint 4 実装時に詰める)**: 固定列の正確な構成 (free tier で locked な cup/breakout の「状態」列の扱い) / 上位強調の具体度合い (何件目まで強調) / 結果リスト件数キャップ (3体合議: ≤100件 + 全件ボタン or 仮想スクロール) / ヒーロー「本日のTOP3」(Sprint 5) / sector chip の in-facet 検索・折りたたみ。
