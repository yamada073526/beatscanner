...
| **status** | ✅ **default ON に promote 済 (v228, 2026-06-20)** — 3体合議 (金融§38 + frontend/dev + qa-dogfooder) verdict 反映後 user「フル」承認で普及。<br>**実装履歴**: ①`6f089aa` gradual-riser path 実装 (flag default OFF) ②`cf7d897` frontend dogfood 配線 ③**v228 promote**: backend `get_technical(cup_gradual=True)` default ON + frontend `getCupGradualFlag()` default true + `api.js` 常に `&cup_gradual=0/1` 明示送信 (kill-switch `?cup_gradual=0` を確実化)。<br>**v228 4必須条件 (合議)**: (1) **sustained guard 補強** — 旧「窓中間点1点 ≤ prior_end」は先行スパイク後横ばいを除外できない死角があり (金融指摘)、**窓3分割で各区画 median 単調増 (m1<m2<m3)** に置換し round-trip/V字/スパイク後横ばいを構造排除。(2) §38 ラベル「過延伸」明示 (`stateCompassText.js` '高値圏'→'高値圏(過延伸)'、`ScreenerPane` '高値圏突破'→'高値圏突破(過延伸)')。(3) Pane3 extended chip に **pivot 乖離率を常時 inline 表示** (hover 依存を脱する)。(4) extended chip `tone="muted"`→`tone="warning"` (amber) で clean cup と視覚区別。<br>**本番検証 (commit `b272c2a` deploy 後 curl, 2026-06-20)**: UBS default ON = `detected=true`・`breakout_extended`・pivot $47.41 (median guard 補強後も検出維持・regression なし) / UBS `?cup_gradual=0` = `detected=false`・`no_prior_uptrend:16` (**kill-switch 動作確認**) / WMT・TM = `formation` 維持 / MS・GE = `breakout_extended` 維持 / **TSLA = 非検出に変化** (ON/OFF とも `no_prior_uptrend:9` 一致 = 補強 median guard が TSLA の gradual rescue を弾いた = 旧 weak guard の false positive を除去、金融 verdict 実証)。RS も self=49/中位/univ=60 で「下位1%」消滅維持。件数: 旧 weak guard ON=21 → 補強後 ~20 (TSLA false positive -1)。<br>**後続 workstream**: ✅ **nightly 配線完了 (2026-06-20, commit `f581fb3`)** — nightly `_scan_one` の `_detect_cup_handle` 呼出 (並列 wc>1 [main.py:16663](backend/app/main.py:16663) + sequential wc=1 [main.py:16737](backend/app/main.py:16737) 両 path) に `allow_gradual_riser=True` を追加し per-ticker (14143) と検出ロジックを一致。**検証**: dry_run で UBS 単独 detected=1 / 手動 GHA nightly (wc=3 並列 path) 完走後 `cup-handle?filter=cup` total_count **444→527 (+83 = gradual 分)**・freshness gate pass。⚠️ **v228 の「total_count=0」前提は誤診だった**: `filter=cup` 指定では strict cup が既に persist 済 (444件)、`filter` なし (デフォルト) が0なのは API 仕様。真の欠落は nightly の gradual flag 未配線の1点のみ (仮説B persist 失敗 / 仮説A cron 未発火 とも否定)。extended amber **着地** (v229, ScreenerPane `d2f221c` + CustomScreenerPanel `51623f6` 両 view: badge span / Chip tone を amber に, per-ticker extended chip と一貫)。⚠️ **authed dogfood で判明**: ①screener tab の主 view は **ScreenerPane** (modal 専用でなく `d2f221c` が本命) ②extended は backend `_STATE_PRIORITY`=5 で**構造的に後方固定** (breakout_confirmed=0 → … → extended=5) → ScreenerPane は top5 表示なので **clean cup が5件以上の日は extended が screener に出ない** (設計上の必然・§38 整合)。amber 実装は正しいが表示頻度は構造的に低く、「extended の見せ方 (現状ほぼ非表示 / 押し目待ち専用枠 / amber 注意)」は色でなく**情報設計の論点** → [[project_screener_tab_redesign]] (screener tab 全体再設計) でまとめて扱う (小 polish ROI 減衰の実例)。 |
...
| ✅ 済 | **D1** | cup gradual-riser を default ON に promote 済 (v228) + **nightly 配線完了 (2026-06-20 `f581fb3`)→screener total 444→527 (+83)**。sustained guard 補強 (median 単調増) + §38ラベル/乖離率/tone | **3体合議 + user承認** |
