...
| `--ring-glow` | `0 0 20px rgba(56,189,248,.20), 0 0 8px rgba(56,189,248,.10)` | `0 0 24px rgba(56,189,248,.30), 0 0 10px rgba(56,189,248,.16)` | EarningsRing 外周 subtle cyan glow (brand emphasis 専用、chandelier 演出用) |
...
### 7-C. Weight Unification Rule (handover v82 multi-review 6 体合議 Q4 verdict)

**fw500 固定 (fw600 禁止)** — Heading / Subsection / Label / 表内 cell (Stat 大以外)。

- 「section 見出し」 「table header」 「fiscal quarter label」 「table cell の actual EPS」 等は **すべて fw500**。
- fw600 は **Display tier (28-32px の Hero 見出し) でのみ許容**。 14px / 11px の中見出し / chip 等は fw500。
- 例外: `Stat 大` (株価 / EPS / % 変動の hero display) は §7-B に従い **fw700**。
- 強調差別化は **色** (`text-primary` vs `text-secondary` vs `text-muted`) で表現、 weight で差をつけない。
- **既存違反箇所 (本 rule 確立時点)**: `ConferenceAnalysis.jsx` L74/L78/L222/L257/L263 を Phase 1 で fw500 統一済。

**Tabular Nums P0 ルール**:

- 全ての数値表示 (株価 / EPS / 売上 / surprise % / 株式数 / 期間ラベル) は `font-variant-numeric: tabular-nums` または Tailwind `tabular-nums` class を **必須**。
- 既存適用箇所: `index.css` の `.section-heading-count` / `.bs-stat-hero-value` / `.ticker-row-v2 .row-price` 等 (Phase 1 時点で 9 箇所)。
- 新規数値表示 component を作るときは必ず `tabular-nums` を付与 (= フォントが等幅でなくても桁ぞろえが効く)。
- **既存違反**: `HistoryChart.jsx` L113 の `shares_diluted` を Phase 1 で適用済。 残違反は handover v82 Phase 2+ で続行。

### 7-D. Sprint 1 (Phase 2) 確定 token + 適用ルール (2026-05-20)

> **Vision-eval target**: typography_grid 78 → 90 (3 run mean)。5 原則 §1 §3 + Aman 級「洗練さ」(数値が「キリッと揃う」) を実装層に実体化。

#### 7-D-1. Typography 6 階層 CSS token (index.css :root で定義)

| Token | 値 | 用途 |
|---|---|---|
| `--text-display` | `32px` | Hero 主見出し (verdict badge) |
| `--text-h1` | `24px` | 銘柄 ticker / 現在値 |
| `--text-h2` | `18px` | section 見出し (SectionHeader) |
| `--text-body-lg` | `16px` | 主要 body (SummaryBrief / GuidanceCard) |
| `--text-body` | `14px` | standard body |
| `--text-caption` | `12px` | caption / badge / unit / metadata |

raw px 直書きをこれらのトークンに統一 (8 Token 使用ポリシー §3 準拠)。

#### 7-D-2. Pane 3 utility class 一覧 (index.css で定義)

| class | 効果 | 適用先 |
|---|---|---|
| `.pane3-numeric` | tabular-nums + tnum + lh1.05 + ls0.01em | 全数値 wrapper |
| `.pane3-numeric-tight` | tabular-nums + tnum + lh1.05 + ls-0.01em + fw700 | Stat 大 |
| `.pane3-section-heading` | --text-h2 + fw500 + lh1.2 + ls-0.015em | SectionHeader h2 |
| `.pane3-body-lg` | --text-body-lg + lh1.6 + ls0.02em | SummaryBrief 等 body |
| `.pane3-caption` | --text-caption + fw500 + lh1.4 | 補足テキスト |
| `.earnings-history-tooltip-value` | tabular-nums + fw700 + lh1.05 | EarningsHistoryChart tooltip |
| `.earnings-history-yoy-badge` | tabular-nums + 10px + fw700 | X 軸直下 YoY badge |
| `.triage-numeric` | tabular-nums + lh1.05 | TriageBanner 数値 |

#### 7-D-3. CSS class 昇格 (Sprint 1)

`Stat.jsx` の inline style (fontWeight/lineHeight/letterSpacing) を `.ds-stat__value` / `.ds-stat__label` CSS class に移管済。
- `.ds-stat__value`: fw700 / lh1.05 / ls-0.01em / tabular-nums / --text-h1 (index.css で定義)
- `.ds-stat__label`: fw500 / lh1.4 / ls0.06em / uppercase / text-muted (index.css で定義)
- `SectionHeader.jsx` の h2: `.pane3-section-heading` class 経由で --text-h2 token 参照

#### 7-D-4. Cascading 二重防御

`.ds-judgment-detail` (Pane 3 root) に `font-variant-numeric: tabular-nums` をルートレベルで適用 (index.css)。
個別 component の `.pane3-numeric` class と二重防御になるが副作用なし。
SummaryBrief `.summary-brief-section` にも cascading 適用済。
